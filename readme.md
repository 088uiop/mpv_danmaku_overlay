...
| type | 说明 | 参数示例 |
| --- | --- | --- |
| `load_ass` | 加载 ASS 字幕文件路径，成功后回 `ready` | `{"type":"load_ass","filepath":"C:\\path\\danmaku.ass"}` |
| `sync` | 播放进度/状态同步，仅 seek、暂停、变速、暂停恢复时发送 | `{"type":"sync","time_pos":12.34,"duration":300,`<br>`"paused":false,"speed":1,"seeking":false}` |
...
| `set_config` | 渲染参数：透明度、字体缩放、弹幕上限 | `{"type":"set_config","opacity":0.8,"font_scale":1.0,"max_danmaku":200}` |
